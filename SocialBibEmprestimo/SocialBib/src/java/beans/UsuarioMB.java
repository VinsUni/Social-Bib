...
public class UsuarioMB {
    ...
    public String excluir() {
        try {
            EmprestimoJpaController daoEmprestimo = new EmprestimoJpaController(EMF.getEntityManagerFactory());
            MensagemJpaController daoMensagem = new MensagemJpaController(EMF.getEntityManagerFactory());
            LoginMB lmb = FacesUtil.getLoginMB();
            usuario.setId(lmb.getUsuario().getId());
            
            if(daoEmprestimo.temDebito(usuario)){
                FacesUtil.adicionarMensagem("Você não pode excluir sua conta porque"
                                            + " tem empréstimos em andamento.");
                return null;
            }
            
            for(Emprestimo e:daoEmprestimo.findEmprestimoEntities(usuario)){
                daoEmprestimo.destroy(e.getId());
            }
            
            for(Mensagem m:daoMensagem.findTodasAsMensagens(usuario)){
                daoMensagem.destroy(m.getId());
            }
